...
export const roomController = {
  addRoom: async (req, res) => {
    try {
      ...
      const room = await roomsModel.create({
        ...req.body,
        roomNumber: rooms ? rooms.roomNumber + 1 : 1,
      });
  ...
  addRoomBooking: async (req, res) => {
    try {
      ...
      console.log(checkIn, checkOut);
      const roomBooking = await roomBookingModel.create({
        ...obj,
        checkIn,
        checkOut,
      });
  ...
  getRoomBookings: async (req, res) => {
    try {
      const bookings = await roomBookingModel
        .find({})
        .populate("room")
        .populate("addedBy")
        .sort({ createdAt: -1 });
  ...
  updateRoom: async (req, res) => {
    try {
      const room = await roomsModel.updateOne(
        { _id: req.params.id },
        { ...req.body }
      );
  ...
  updateRoomBooking: async (req, res) => {
    try {
      const booking = await roomBookingModel.updateOne(
        { _id: req.params.id },
        { ...req.body }
      );
