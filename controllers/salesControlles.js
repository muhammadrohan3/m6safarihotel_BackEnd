...
export const salesController = {
  addFoodSales: async (req, res) => {
    try {
      ...
      const foodSales = await foodSalesModel.create({
        ...req.body,
        price: food.price,
      });
  ...
  getFoodSales: async (req, res) => {
    try {
      const foodSales = await foodSalesModel
        .find({})
        .populate("foodItem")
        .populate("addedBy")
        .sort({ createdAt: -1 });
  ...
  addDrinkSales: async (req, res) => {
    try {
      ...
      await drinksModel.updateOne(
        { _id: req.body.drinkItem },
  ...
  getDrinkSales: async (req, res) => {
    try {
      const sales = await drinkSalesModel
        .find({})
        .populate("drinkItem")
        .populate("addedBy")
        .sort({ createdAt: -1 });
  ...
  addStock: async (req, res) => {
    try {
      const updateStock = await drinksModel.updateOne(
        { _id: req.body.drinkItem },
  ...
  getStock: async (req, res) => {
    try {
      const stock = await drinkStockModel
        .find({})
        .populate("drinkItem")
        .populate("addedBy")
        .sort({ createdAt: -1 });
  ...
  deleteDrink: async (req, res) => {
    try {
      const drink = await drinksModel.updateOne(
        { _id: req.params.id },
        { status: false }
      );
  ...
  updateFoodSales: async (req, res) => {
    try {
      const foodSales = await foodSalesModel.updateOne(
        { _id: req.params.id },
        req.body
      );
  ...
  updateDrinkSales: async (req, res) => {
    try {
      ...
      await drinksModel.updateOne(
        { _id: drinkSale.drinkItem },
        { $inc: { stock: drinkSale.quantity } }
      );
      await drinksModel.updateOne(
        { _id: req.body.drinkItem },
        { $inc: { stock: -req.body.quantity } }
      );
      const drinkSales = await drinkSalesModel.updateOne(
        { _id: req.params.id },
        req.body
      );
  ...
  deleteDrinkSales: async (req, res) => {
    try {
      ...
      const drinkSales = await drinkSalesModel.deleteOne({
        _id: req.params.id,
      });
  ...
  updateDrinkStock: async (req, res) => {
    try {
      ...
      const drinkStock = await drinkStockModel.updateOne(
        { _id: req.params.id },
        req.body
      );
      await drinksModel.updateOne(
  ...
  deleteDrinkStock: async (req, res) => {
    try {
      const stock = await drinkStockModel.findOne({ _id: req.params.id });
      const drinkStock = await drinkStockModel.deleteOne({
        _id: req.params.id,
      });
      await drinksModel.updateOne(
        { _id: stock.drinkItem },
